...
export const getAmountOut = (
	...
	const amount0Bigint = stringToBigIntWithDecimals(value, decimalsFrom);

	const orderedReserve0 = isOrdered ? reserve0 : reserve1;

	const orderedReserve1 = isOrdered ? reserve1 : reserve0;

	const num = (orderedReserve1 * BigInt(997) * amount0Bigint) / BigInt(1000);

	const denum = orderedReserve0 + (amount0Bigint * BigInt(997)) / BigInt(1000);

	const amountToBigint = num / denum;

	return displayDecimalNumber(amountToBigint, decimalsTo);
};
...
export const getAmountIn = (
	...
	const amountToBigint = stringToBigIntWithDecimals(value, decimalsTo);
	const orderedReserve0 = isOrdered ? reserve0 : reserve1;
	const orderedReserve1 = isOrdered ? reserve1 : reserve0;

	const num = orderedReserve0 * amountToBigint * BigInt(1000);

	const denum = (orderedReserve1 - amountToBigint) * BigInt(997);

	const amountFromBigint = num / denum + BigInt(1);

	return displayDecimalNumber(amountFromBigint, decimalsFrom);
};
